...
def equal(u,v):
    ...
    assert u.D == v.D
    return all(getitem(u, d) == getitem(v, d) for d in v.D) 

def add(u,v):
    "Returns the sum of the two vectors"
    assert u.D == v.D
    return Vec(v.D, {d:getitem(v, d)+getitem(u, d) for d in u.D})

def dot(u,v):
    "Returns the dot product of the two vectors"
    assert u.D == v.D
    return sum(v[d]*u[d] for d in u.D);
...
def neg(v):
    ...
    return Vec(v.D, {d:v[d]*-1 for d in v.D})
    pass
