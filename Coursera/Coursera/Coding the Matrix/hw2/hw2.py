...
def vec_sum(veclist, D): 
    ...
    True
    '''
    testVec = sum(0 + x for x in veclist)
...
def scale_vecs(vecdict):
    ...
    return [Vec(x.D, x.f) for x in [1/x*y for (x, y) in vecdict.items()]]
...
def GF2_span(D, L):
    ...
    for (x,y) in test1:
        ...
        output.append(scalar_mul(L[0], x) + scalar_mul(L[1], y) )
