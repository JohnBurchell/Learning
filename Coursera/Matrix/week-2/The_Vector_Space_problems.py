...
def vec_sum(veclist, D):
    ...
    return sum(veclist, Vec(D, {x:0 for x in D}))
...
def scale_vecs(vecdict):
    ...
    [True, True]
    '''
    return [vecdict[x] / x for x in [x for x in vecdict]]
...
def GF2_span(D, S):
    ...
    True
    '''
    return {sum([y * z for (y,z) in zip(x,S)]) for x in product({0,one},repeat=len(S))} if len(S) != 0 else Vec(D, {})
