...
def lin_comb_mat_vec_mult(M, v):
    ...
    '''
    assert(M.D[1] == v.D)
    return sum([v[x]* mat2coldict(M)[x] for x in v.f])
...
def lin_comb_vec_mat_mult(v, M):
    ...
    '''
    assert(v.D == M.D[0])
    return sum([v[x]* mat2rowdict(M)[x] for x in v.D])
...
def dot_product_mat_vec_mult(M, v):
    ...
    '''
    assert(M.D[1] == v.D)
    return Vec(M.D[0], {x:v*mat2rowdict(M)[x] for x in M.D[0]})
...
def dot_product_vec_mat_mult(v, M):
    ...
    Examples:
      ...
      '''
    assert(v.D == M.D[0])
    return Vec(M.D[1], {x:v*mat2coldict(M)[x] for x in M.D[1]})
