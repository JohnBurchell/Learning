...
indep_vec_2 = Vec({0,1,2}, {0:0, 1:2, 2:0})
indep_vec_3 = Vec({0,1,2}, {0:0, 1:0, 2:3})
...
def is_superfluous(S, v):
    ...
    S1.remove(v)
    M = coldict2mat(list(S1))
    return (v - M * solve(M,v)).is_almost_zero()
...
def exchange(S, A, z):
    ...
    Examples:
        ...
        True
    '''
    result = vec2rep(list(S), z)
    for x in result.D:
        if result[x] != 0 and list(S)[x] in S - A:
            return list(S)[x]
    return None
