...
def equal(A, B):
    ...
    """
    assert A.D == B.D
    return all(A[x] == B[x] for x in A.f.keys() | B.f.keys())
...
def add(A, B):
    ...
    """
    assert A.D == B.D
    return Mat(A.D, {(x,y):A[x,y] + B[x,y] for x,y in A.f.keys() | B.f.keys()})
...
def scalar_mul(M, x):
    ...
    True
    """
    return Mat(M.D, {(k,v):M[k,v] * x for k,v in M.f.keys()})
...
def transpose(M):
    ...
    True
    """
    return Mat((M.D[1],M.D[0]),{(y,x):M[x,y] for x,y in M.f.keys()})
...
def vector_matrix_mul(v, M):
    ...
    for (i,j) in M.f.keys():
        return_vec[j] = return_vec[j] + M[i,j] * v[i]
...
def matrix_vector_mul(M, v):
    ...
    for (i,j) in M.f.keys():
        return_vec[i] = return_vec[i] + M[i,j] * v[j]
