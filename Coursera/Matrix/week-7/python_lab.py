...
representation_dict = {x:(x //base**2 % base, x //base**1 % base, x % base) for x in [(x*(base**2)+(y*base)+z) for x in digits for y in digits for z in digits]}
...
listdict2dict = {names[x]:id2salary[x] if x in id2salary else 'NOT PRESENT' for x in range(len(names))}
